...
#define TURN_DISTANCE (float)3.0
...
{
	PIDPositionSet(SubMod(angle_dest,angle)*RAD2PULSES,DELTA_POS_STRAIGHT);
...
void TurnRight(float x, float y, float angle)
{
	...
	if (fabsf(SubMod(angle,angle_dest))>PI/2)
		PIDPositionSet((int32_t)(-TURN_ANGLE*DEGREE2PULSES),DELTA_POS_TURN);
	else
		PIDPositionSet(0,DELTA_POS_TURN);
}
